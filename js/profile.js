...
async function profile(message, client, id, avatar, args1) {
    ...
    db.query(query, async function (err, result) {
        ...
        let def = getDefense(data.basic_def, data.level, data.helmetDef, data.shirtDef, data.pantsDef, data.bonus_armor_set, data.helmet_modifier, data.shirt_modifier, data.pants_modifier, playerBuff.buff_def);
        let weaponModifier = data.attack * data.weapon_modifier;
        let attack = getAttack(data.basic_attack, data.attack, data.level, data.weapon_modifier, playerBuff.buff_attack);
        let maxHp = getMaxHP(data.basic_hp,data.level, playerBuff.buff_max_hp);
        ...
        let buffEmojiData = await queryData(`SELECT cfg_buff.emoji 
            FROM active_buff
            LEFT JOIN cfg_buff ON (active_buff.buff_id=cfg_buff.id)
            WHERE player_id=${message.author.id} AND expire_time>${getTimeNow()} LIMIT 10`);
        buffEmojiData = buffEmojiData.length > 0 ? buffEmojiData : undefined;
        let activeBuff = '';
        if (buffEmojiData) {
            let buffEmoji = '';
            for (let i = 0; i < buffEmojiData.length; i++) {
                if(buffEmoji) { buffEmoji += ' '}
                buffEmoji += buffEmojiData[i].emoji;
            }
            activeBuff = `**Buff** : ${buffEmoji}`
        }
