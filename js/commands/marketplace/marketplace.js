    ...
    var marketplace = async function (page) {
        ...
        message.channel.send(embed)
            .then(() => {
                // let filter = false;
                // if (args[0] == 'list') {
                  let filter = m => m.author.id === message.author.id;// && (m.content.toLowerCase() == 'claim' || m.content.toLowerCase() == 'mn' || m.content.toLowerCase() == 'mb');
                // } else {
            ...
            message.channel.awaitMessages(filter, {
                ...
                .then( async message => {
                    ...
                    if (args[0] == 'list' && message.content.toLowerCase() == 'claim') {
                        if (profit <= 0) { return message.channel.send(`There is no item sold on your listing!`) };
                        // ADD GOLD
                        profit = await cekMaxGold(message, message.author.id, profit);
                        ...
                        if (profit < 0) {
                            return message.channel.send(`Your gold has been reduced to {emojiCharacter.gold2}**${currencyFormat(profit)}**`);    
                        }
